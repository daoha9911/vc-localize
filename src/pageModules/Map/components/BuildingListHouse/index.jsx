...
import { getAllEstatesHouseFromId } from '../../services/getFilterEstates';
import styles from './index.less';

const BuildingListHouse = ({ id }) => {
  const { setFilter } = useQueryLocation();
  const getList = useCallback(({ current, pageSize }) => {
    return getAllEstatesHouseFromId({ id, current, pageSize });
  });
        ...
        <>
          <div className="mb-4 flex items-center">
            ...
            <h2 className={styles.customHeaderH2}>
              Nhà đang bán tại {Array.isArray(data?.data) ? `${data?.data[0]?.address}` : 'đây'}
            </h2>
          </div>
          <Row gutter={[32, 32]}>
            {(data || []).map((d) => (
...
export default BuildingListHouse;
